        ...
        <div class="pagetitle" style="margin-left: 20px;">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"><a> Manage Process</a></li>
                </ol>
            </nav>
        </div>
        ...
        <div class="row justify-content-center">
            <div class="card-body">
                <table class="table table-border datatable " style="border: 1px solid black">
                    ...
                    <tbody>
                        @foreach ($processes as $processes => $item)
                            <tr>
                                <th>{{ $processes + 1 }}</a></th>
                                <th>{{ $item->id }}</a></th>
                                <td>{{ $item->process }}</td>
                                <td><a class="datatable-sorter"></a>{{ $item->process_code }}</td>
                                <td>{{ $item->detail }}</td>
                                <td>
                                    @if ($item->is_active)
                                        <p>Active</p>
                                ...
                                <td>
                                    <form action="" method="POST">
                                        ...
                                        <a class="btn btn-primary" href="{{route('process.edit',$item->id)}}">Edit</a>
